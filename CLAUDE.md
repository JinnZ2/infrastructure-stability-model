...
- Any external number added to a schema gets an entry in `SOURCES.md` saying what was actually read, plus its reference period, depth/dimension, spatial scope, instrument era, and what "not that bad" was compared against. A number without a baseline is not usable.
- **Before reporting a result, perturb the input at the innermost function and confirm the output moves.** Threading an argument through a signature is not evidence it is used — the ENSO forcing was wired into the readout and not the integrator, and a full set of numbers was published from it (ledger `F-018`).
- **State the sample size at which a figure resolves.** A ratio whose seed-to-seed range exceeds its own effect is not a result (ledger `F-020`). Report distributions and tails, not just medians and p90s.
- Thresholds in this repo are **levels**, and the model is a pure integrator without feedback — it cannot express a rate-triggered condition (ledger `F-019`). Do not describe it as capable of cascade or phase change; every mechanism that would produce one lives outside the ODE.
